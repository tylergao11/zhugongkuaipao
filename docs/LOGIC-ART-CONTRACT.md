...
- main.js：拖卡部署。武将技能范围内有合法目标时自动释放，底部卡牌不再点击施放。脚底抹油是被动，读数一行小字。没有护驾按钮。点击单位菜单可拆卸、转向、换防或升级。BOSS 关铜锣走左上角按钮；拖动画面和取消触摸不发动机关。
...
- 坐骑：教学关 level.mount 提供位置和倍率，game.mount.claimed 表示已领取；liu.mounted 或 liu.dash 都画的卢骑乘。的卢图集每关进入都加载。BOSS 关没有开场的卢对白。
